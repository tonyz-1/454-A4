...
//
// The <ft2build.h> should be on your 'include' path (using -I on the C++ compile line)
...

{
  ...
  if(FT_New_Face(ft, ttf_file, 0, &face)) {
    ...
    fprintf(stderr, "Could not open font.  Current working directory is %s\n", path );
    exit(1);
  }
  ...
  gpu = new GPUProgram();
  gpu->init( vertexShader, fragmentShader, "font.cpp" );
#endif
}
