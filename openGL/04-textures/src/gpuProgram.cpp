...

{
  ...
  if (fileName != NULL) {
    ...
    if (file == NULL) {
      ...
      std::cerr << "File " << fileName << " could not be opened.  Current working directory is " << path << std::endl;
      text = (char *) "";
...

{
  ...
  if (vsText == NULL) {
    char path[PATH_MAX];
    _getcwd( path, PATH_MAX );
    std::cerr << "Vertex shader file '" << vsFile << "' not found.  Current working directory is " << path << std::endl;
    return;
  }
  ...
  if (fsText == NULL) {
    char path[PATH_MAX];
    _getcwd( path, PATH_MAX );
    std::cerr << "Fragment shader file '" << fsFile << "' not found.  Current working directory is " << path << std::endl;
    return;
  }
