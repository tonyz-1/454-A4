...
mediump float getMipMapLevel( sampler2D texUnitID, mediump vec2 texCoords )

{
  // For GLSL >= 4.0
  //
  // return textureQueryLod( texUnitID, texCoords ).x

  // For GLSL < 4.0

  mediump vec2  dx_vtc        = vec2( dFdx(texCoords.x), dFdx(texCoords.y) );
  mediump vec2  dy_vtc        = vec2( dFdy(texCoords.x), dFdy(texCoords.y) );
