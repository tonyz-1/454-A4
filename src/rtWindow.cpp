...

{
  if (action == GLFW_PRESS)
    ...
    case '+':
    case '=':
      scene->numRaySamples *= sqrt(2);
      cout << "num samples " << scene->maxDepth << endl; 
      viewpointChanged = true;
      redisplay = true;
      break;
    ...
    case '_':
      scene->numRaySamples /= sqrt(2);
      if (scene->numRaySamples < 1) 
	scene->numRaySamples = 1;
      cout << "num samples " << scene->maxDepth << endl; 
      viewpointChanged = true;
      redisplay = true;
      break;
    ...
    case 'G':
      if (mods & GLFW_MOD_SHIFT)
	scene->glossinessFactor *= 2;
      else
	scene->glossinessFactor /= 2;
      redisplay = true;
      cout << "glossiness factor " << scene->glossinessFactor << endl;
      break;
    ...
    case 'R':
      scene->russianRoulette = !scene->russianRoulette;
      redisplay = true;
      cout << "Russian Roulette " << (scene->russianRoulette ? "on" : "off") << endl;
      break;
	...
	<< "z     toggle pixel zooming (then click or click-and-drag mouse on pixels)" << endl
	...
	<< "CTRL  left mouse click/drag  - zoom pixels at mouse" << endl
